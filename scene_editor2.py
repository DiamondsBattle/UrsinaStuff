...
class EditorIcon(Draggable):
    def __init__(self, **kwargs):
        ...
        self.sprite = Entity(parent=self, add_to_scene_entities=False, model='quad', texture='circle', scale=.85,
                             color=color.white)
    ...
    def input(self, key):
        ...
        if key == 'left mouse up':
            ...
            for icon in self.scene_editor.editor_icons:
                ...
                if pos.x > self.x and pos.x < self.x + abs(self.scale_x) and pos.y > self.y and pos.y < self.y + abs(
                        self.scale_y):
...
class SceneEditor(Entity):
    def __init__(self):
        super().__init__()
        self.world_grid = Entity(parent=self, model=Grid(32, 32), scale=32, rotation_x=90, color=color.white33,
        ...
        self.view_modes = [DropdownMenuButton(text=str(i), on_click=Func(self.changeRenderMode, str(i))) for i in
                           window.render_modes]
        ...
        self.top_menu = DropdownMenu('Menu', buttons=(
            ...
            DropdownMenu('Entity', buttons=(
                ...
                DropdownMenu('Help', buttons=(
                    DropdownMenuButton('Tutorial',
                                       on_click=Func(browse, 'https://www.ursinaengine.org/ursina_for_dummies.html')),
                    DropdownMenuButton('API Reference',
                                       on_click=Func(browse, 'https://www.ursinaengine.org/cheat_sheet.html')),
                )),
            )),
